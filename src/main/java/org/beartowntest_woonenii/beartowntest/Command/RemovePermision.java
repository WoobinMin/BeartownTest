...
public class RemovePermision implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        if(commandSender instanceof Player)
        {
            ...
            if(strings[0].equalsIgnoreCase("use"))
            {
                ...
                player.sendMessage(Component.text("/giveaway 권한을 잃었습니다."));
            }
            else if(strings[0].equalsIgnoreCase("admin"))
            {
                playerConfig.set("players."+player.getName()+".giveaway.admin", false);
                player.sendMessage(Component.text("/giveawayadmin 권한을 잃었습니다."));
            }
